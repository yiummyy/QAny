...
class PerfSample:
    ...
    item_id: str = ""
    ttft_ms: float = 0.0       # Time To First Token（首个 MESSAGE chunk 到达）
    total_ms: float = 0.0       # 总响应时间（DONE 事件到达）
    token_count: int = 0        # 输出 token 数（chunk 计数）
...
class PerfReport:
    ...
    # concurrency → { qps, ttft_p50, ttft_p99, total_p50, total_p99, success_rate, ... }
    ...
    try:
        async with client.stream(
            ...
            async for line in resp.aiter_lines():
                ...
                if line.startswith("data:") and line.startswith("event: message"):
                    # 上一行是 event: message，这一行是 data
                    pass  # token 计数在 data 行
                ...
                if line.startswith("event: error"):
                    ...
                    try:
                        err_data = json.loads(data_str.split(":", 1)[1].strip()) if ":" in data_str else {}
...
async def benchmark_concurrency(
    ...
    每个并发等级创建一个连接池，复用连接以模拟真实场景.
    """
    report = PerfReport()

    for concurrency in concurrency_levels:
        # 轮转取题（避免相同 query 被 FAQ 缓存命中）
