...
from eval.performance import PerfReport
from eval.quality import QualityReport
from eval.retrieval import RetrievalReport


# ── 检索质量报告 ──────────────────────────────────────


def retrieval_to_dict(report: RetrievalReport) -> dict[str, Any]:
    return {
        "summary": {
...
def retrieval_md(report: RetrievalReport) -> str:
    lines = [
...
def perf_to_dict(report: PerfReport) -> dict[str, Any]:
    return {
        "concurrency_levels": report.concurrency_levels,
    }


def perf_md(report: PerfReport) -> str:
    lines = [
    ...
    if ttft_values:
        lines += [
            ...
            f"- **P50:** {_p(ttft_values, 50):.0f}ms",
            f"- **P95:** {_p(ttft_values, 95):.0f}ms",
            f"- **P99:** {_p(ttft_values, 99):.0f}ms",
...
def quality_to_dict(report: QualityReport) -> dict[str, Any]:
    return {
        "summary": {
            "total_queries": report.total_queries,
            "errors": report.errors,
...
def quality_md(report: QualityReport) -> str:
    lines = [
    ...
    for pq in report.per_query:
        if pq.status != "success":
            ...
            lines.append(
                f"| {pq.item_id} | {pq.f1:.3f} | {'✓' if pq.exact_match else ''} | "
                f"{pq.rouge_l:.3f} | {'✓' if pq.factual_passed else ''} | "
...
def _p(values: list[float], pct: float) -> float:
    from eval.performance import percentile
